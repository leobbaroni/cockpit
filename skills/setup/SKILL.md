...
| **ComfyUI + comfy-mcp** | **Generation on the user's own GPU, free per run** — plus a hosted partner catalog reachable from the same surface. maestro's `generative-engines.md` probes it with `server_info`, `list_partner_models`, and `search_templates`; without it, the local/free route simply doesn't exist and every generation is billed | `pip install comfy-cli comfy-mcp` (needs Python ≥ 3.10), then `comfy install` for a workspace and `comfy launch` to run it. Register the MCP with the `comfy-mcp` command over stdio. Skills: `/plugin marketplace add Comfy-Org/comfy-skills`. **No usable GPU?** `/plugin install comfy-cloud@comfy-skills` runs the same workflows on Comfy's hardware — subscription required, and it sees a standard catalog rather than this machine's custom nodes. **Check the GPU, not just the install** — `server_info` reports VRAM, and video models want 16+ GB |
| **Higgsfield** | Hosted image/video/3D/audio generation across many vendors, Soul ID identity locking, Marketing Studio ads, and Virality Predictor scoring. maestro's generative modules are distilled from its first-party skills, so it is the baseline they assume | `/plugin marketplace add higgsfield-ai/skills` then `/plugin install higgsfield@higgsfield` (or `npx skills add higgsfield-ai/skills`). MCP: `claude mcp add --transport http --scope user higgsfield https://mcp.higgsfield.ai/mcp`. CLI: `curl -fsSL https://raw.githubusercontent.com/higgsfield-ai/cli/main/install.sh \| sh`, then `higgsfield auth login`. **Paid** — generations spend credits, so say so before any batch |
