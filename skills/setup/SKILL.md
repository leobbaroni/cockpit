...
| **ComfyUI + comfy-mcp** | **Generation on the user's own GPU, free per run** — plus a hosted partner catalog reachable from the same surface. maestro's `generative-engines.md` probes it with `server_info`, `list_partner_models`, and `search_templates`; without it, the local/free route simply doesn't exist and every generation is billed | `pip install comfy-cli comfy-mcp` (needs Python ≥ 3.10), then `comfy install` for a workspace and `comfy launch` to run it. Register the MCP with the `comfy-mcp` command over stdio. Skills: `/plugin marketplace add Comfy-Org/comfy-skills`. **Check the GPU, not just the install** — `server_info` reports VRAM, and video models want 16+ GB |
