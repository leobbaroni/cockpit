...
| Skill | Discipline |
|---|---|
| **pilot** | The flagship, run as a **task graph** rather than a prompt. A ~75-line router detects the phase (PLAN / KICKOFF / BUILD / REVIEW / IMPROVE / DEBUG / DELIVER) and loads **one node** for it — so a build turn costs about a fifth of what the old monolith did. `STATE.md` is the state contract: which files carry work between phases, who may write each, and which are **frozen anchors** the build is judged against rather than allowed to rewrite. `CONTRACT.md` is the four-gate execution discipline: restate the ask, touch only what you named, the rationalization tripwire, and "done" meaning a check you ran in the same message. Fan-out goes through `orchestrate`, never from pilot. |
